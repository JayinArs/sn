...
@section('top')
    <a href="{{ route('org.create') }}" class="btn btn-success">Add New</a>
@endsection
            ...
            <div class="panel panel-default">
                <div class="panel-body pb0">
                    <div class="table-responsive table-bordered mb-lg">
                        <table class="table table-striped table-bordered table-bordered-force" id="organizations-table"
                               style="width: 100%;">
                            <thead>
                            <tr>
                                <th>Org. ID#</th>
...
<script>
    $(function () {
        ...
        $(document).on('click', '.table-actions a[data-action]', function (e) {
            ...
            var data = $org_table.row($btn.parents('tr')).data();

            if ($action === 'delete') {
                $delete_modal.on('show.bs.modal', function () {
                    $delete_modal.find('#delete-modal-message').html('Are you sure to delete this organization?');
                    $delete_modal_ids.val(data['id']);
                });
                $delete_modal.modal('show');
            } else if ($action === 'detail') {
                window.location.href = window.custom.url + '/org/' + data['id'] + '/edit';
            }
        });
        ...
        $delete_modal.on('click', '#delete-modal-confirmed', function () {
            ...
            $.ajax({
                type: 'DELETE',
                url: '{{ url("admin/org") }}/' + $delete_modal_ids.val(),
